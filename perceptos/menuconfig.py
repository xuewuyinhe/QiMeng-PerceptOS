...
class MenuConfig:
    ...
    def get_value_from_user(self, sc):
        ...
        print(sc.rev_dep)
        print(3)
        print(sc.weak_rev_dep)
        print(4)
        print(sc.implies)
        for selected_symbol, condition in sc.selects:
              print(f"Symbol {sc.name} selects {selected_symbol.name} with condition {condition}")
        #save +="\n"+ ("value = " +sc.str_value)
    ...
    def run(self,cmd):
        ...
        if cmd:
            ...
            if cmd == "help" :
                ...
                save+="pwd                      #打印当前节点\n"
                save+="0                        #数字代表进入第几个子结点\n"
                save+="config_name              #输入config名字，返回详细信息和如何设置   (新增:如何设置该config)\n"
                save+="write config_name y      #config_name是上一次查询的名字，在这里修改(新增:实时修改文件)\n"
                save+="load_config  file_name   #file_name 是想要加载的名字，比如.config  (新增:加载.config文件)\n"
                save+="write_config file_name   #将当前文件保存在file_name                (新增:保存当前文件到file_name)\n"
                save+="last_changes             #返回最近一次config改动之后，变化的节点们\n"
                save+="vis config_name          #config_name 是想要检查可见性的节点名字\n"
                state=True
            elif cmd == "up":  #返回上一级节点
                if len(self.stack) == 0:
            ...
            elif cmd_list[0] == "load_config": #加载config文件
                ...
                try:
                    ...
                    save=e
                    state=False
            elif cmd_list[0] =="write_config":  #保存config文件
                config_filename=cmd_list[1]
                try:
                    ...
                    save=e
                    state = False
            elif cmd in self.kconf.syms: #打印特定config的信息
            ...
            elif cmd_list[0] == "write": #写当前的config命令
                if len(cmd_list) != 3:
